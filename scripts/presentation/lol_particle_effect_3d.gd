...
func _spawn(c: Dictionary, birth: float) -> void:
	...
	if is_mesh:
		var mesh_path := _asset(c["p-mesh"], "glb")
		if not _meshes.has(mesh_path):
			var instance := (load(mesh_path) as PackedScene).instantiate()
			var found := instance.find_children("*", "MeshInstance3D", true, false)
			_meshes[mesh_path] = (found[0] as MeshInstance3D).mesh
			instance.free()
		node.mesh = _meshes[mesh_path]
	else:
...
func _update_particle(p: Dictionary, age: float) -> void:
	...
	node.global_position = (p.origin as Transform3D).origin.lerp(global_position, bound) + global_basis * (offset * _factor)
	...
	if bool(p.mesh) or int(_number(c, "p-type", 1)) == 2:
		...
		if camera != null:
			node.global_basis = camera.global_basis.rotated(camera.global_basis.z, deg_to_rad(angles.x))
	# 缩放只属于粒子 mesh，绝不写回单位。
...
static func _number(c: Dictionary, key: String, fallback: float) -> float:
	return float(String(c[key]).split(" ")[0]) if c.has(key) else fallback

static func _vec3(c: Dictionary, key: String, fallback: Vector3) -> Vector3:
	if not c.has(key): return fallback
	var a := _values(c[key])
	return Vector3(a[0], a[1] if a.size() > 1 else a[0], a[2] if a.size() > 2 else a[0])
...
static func _value4(c: Dictionary, key: String, fallback: Vector4) -> Vector4:
	if not c.has(key): return fallback
	var a := _values(c[key])
	return Vector4(a[0], a[1], a[2], a[3]) if a.size() == 4 else fallback
...
static func _curve3(c: Dictionary, key: String, t: float, fallback: Vector3) -> Vector3:
	var adapted := {}
	for i in range(1, 12):
		if c.has(key + str(i)): adapted[key + str(i)] = String(c[key + str(i)]) + " 1"
	var v := _curve4(adapted, key, t, Vector4(fallback.x, fallback.y, fallback.z, 1))
	return Vector3(v.x, v.y, v.z)

func _probability(c: Dictionary, key: String, fallback: float) -> float:
	var entries := []
	for i in range(1, 12):
		if c.has(key + "P" + str(i)): entries.append(_values(c[key + "P" + str(i)]))
