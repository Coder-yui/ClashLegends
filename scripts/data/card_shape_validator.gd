...
const TEXT_FIELDS := ["attack_wave_visual", "heal_on_hit_name", "on_hit_passive_name", "growth_ranged_id", "growth_melee_id", "deployment_upgrade_id", "icon_path", "rush_spawn_id", "name", "type", "description", "size_tier", "deploy_zone", "spawn_id", "spawn_side", "death_spawn_id", "death_replacement_id", "death_replacement_visual_transition", "timed_revival_id", "timed_revival_visual_transition", "active_buff_projectile_visual", "projectile_visual", "projectile_impact_visual", "deploy_sweep_name", "spell_kind", "active_name", "visual_pre_deploy_scene", "visual_active_buff_scene", "visual_scene_path", "kind", "visual_action", "shape", "full_resource_visual_action", "target_scope", "deployment_formation"]
const BOOL_FIELDS := ["resource_consume_only_full", "terrain_traversal", "deploy_pocket_requires_both_towers", "form_lifetime_after_transition", "form_refresh_on_kill", "selectable", "custom_radius", "is_air", "is_building", "building_only", "can_attack_air", "is_continuous_attack", "deploy_ignore_structures", "show_team_ring", "lifespan_hp_decay", "tower_ruin_foundation", "projectile_spawn_at_edge", "shield_decay", "shield_on_cast_start", "independent_on_creation", "ignore_movement_slow", "ignore_attack_speed_slow", "ground_only", "fan_inner_arc", "projectile_stop_on_hit", "projectile_piercing", "uses_skill_resource", "cast_end_heal_requires_hit", "applies_on_hit_passive", "shockwave_full_only", "global_heal", "copy_member_buff"]
			...
			"events":
				...
				for cue in value[key]:
					...
					for fade in ["fade_in", "fade_out"]:
						if event.has(fade): check(event_path + "." + fade, event[fade], "number", errors)
