...
const TEXT_FIELDS := ["name", "type", "description", "size_tier", "deploy_zone", "spawn_id", "spawn_side", "death_spawn_id", "death_replacement_id", "death_replacement_visual_transition", "timed_revival_id", "timed_revival_visual_transition", "active_buff_projectile_visual", "projectile_visual", "projectile_impact_visual", "deploy_sweep_name", "spell_kind", "active_name", "visual_active_buff_scene", "visual_scene_path", "kind", "visual_action", "shape", "full_resource_visual_action", "target_scope", "deployment_formation"]
const BOOL_FIELDS := ["selectable", "custom_radius", "is_air", "is_building", "building_only", "can_attack_air", "is_continuous_attack", "deploy_ignore_structures", "show_team_ring", "lifespan_hp_decay", "tower_ruin_foundation", "projectile_spawn_at_edge", "cancel_attack_recovery_without_target", "shield_decay", "shield_on_cast_start", "ignore_movement_slow", "ignore_attack_speed_slow", "ground_only", "fan_inner_arc", "projectile_stop_on_hit", "projectile_piercing", "uses_skill_resource", "cast_end_heal_requires_hit", "applies_on_hit_passive", "shockwave_full_only", "global_heal"]
const NUMERIC_ARRAY_FIELDS := ["attack_pattern", "attack_damage_multipliers", "attack_extra_hit_damage_multipliers", "attack_extra_hit_delays", "resource_damage_by_stacks", "resource_hit_damage_sequences", "resource_hit_delay_sequences"]
...
static func validate(label: String, stats: Dictionary, errors: PackedStringArray, skill: bool = false) -> bool:
	...
	for key in stats:
		...
		elif key in ["visual_scene_paths", "resource_visual_actions", "cast_locks"]:
