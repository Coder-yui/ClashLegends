...
建筑另需 `is_building = true`、正数 `footprint_tiles`、`speed = 0`、`lifespan`。`footprint_tiles` 只负责下牌格占用；实际移动寻路、绕行和防穿模统一使用 `radius` 定义的圆柱碰撞，两者不得互相反推。`lifespan_hp_decay = true` 时按 `max_hp / lifespan` 在固定模拟中匀速扣减生命，寿命结束时恰好归零，且自然衰减不算受击。周期召唤使用 `spawn_id/spawn_interval/spawn_count/spawn_side`，亡语召唤使用 `death_spawn_id/death_spawn_count`。召唤引用会在启动前校验。
