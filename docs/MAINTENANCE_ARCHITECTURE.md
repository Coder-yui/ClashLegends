...
技能预热覆盖完整动画库的采样、受击/冰冻/强化叠加材质、强化场景与死亡后续模型；男爵之力的程序覆层随模型材质组合一起预热。绘制样本不注册战斗对象、不施放技能、不播放音频。纯 2D 几何技能图形没有额外外部素材，仍由已有渲染代码按实际事件绘制。
...
本机Compatibility渲染器需要实际可见绘制。预热仍采样全部片段，但相同网格/材质/可见性组合只绘制一次；六种控制/强化材质组合保留实际绘制，force_draw(false)不逐次交换屏幕缓冲，状态之间不额外等待帧；相同初始化需求的样本合并，保留绘制资源与骨骼，停止并移除不再使用的样本动画库，不影响正式模型。男爵之力身体覆层使用程序着色器，强化弹体与出膛/命中使用自制二维几何绘制，不加载或预热原版男爵粒子族。
...
预部署冲击波的命中集合随CommandSchedule条目存活；PreDeploymentSweep提供只读轨迹及扫掠，Main只在权威预部署批次传入战斗对象并提交伤害。客户端计时只推进显示，不传入伤害目标。表现共用轨迹采样，不驱动模拟，实体仍仅在预部署完成后经通用生成入口创建。
