...
| attack_launch | AsheBasicAttack_OnMissileLaunch | 3 | 权威弹体实际生成（基础节点为攻击开始后 0.140625 秒，按攻速缩放并由固定 Tick 派发） |
| attack_hit | AsheBasicAttack_OnHit | 3 | 箭实际抵达并成功命中 |
| active:start | Volley_OnCast | 2 | Spell2 / active 起手 |
| active:release | VolleyAttackWithSound_OnMissileLaunch | 1 | 约 0.163636 秒的权威 Impact 节点，空放也触发 |
| active:hit | VolleyAttack_OnHit | 3 | 箭矢首次真实碰撞并成功伤害，同次施法多目标只播一次 |
...
- 万箭齐发离弦时创建 8 支权威非穿透直线箭并播放 release；实际碰撞成功后才播放 hit，每次施法最多一次。命中音保存原 card_id/form，施法者销毁后仍可重放；通过带事件 ID 的可靠通知去重。音频不驱动碰撞或伤害。
