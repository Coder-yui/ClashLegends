> 整理前快照，归档于 2026-09-13；含已被后续段落替代的数值与判断。当前入口见 [卡牌手册](../../../units/melee_minion.md)。

# 近战兵（`melee_minion`）

当前定义：[`scripts/data/cards/melee_minion.gd`](../../../../scripts/data/cards/melee_minion.gd)，使用 gameplay / visual / card_art / audio 四域；通用接入与验收见 [卡牌设计](../../../CARD_DESIGN.md)。
...
- 当前主动技能数值：消耗 `0` 金币；每个近战兵最多使用 `2` 次；每次使用后冷却 `5.0 s`。
- 持续 `4.0 s`。
...
- 普攻：`Attack1`、`Attack2`。
- 死亡：`Death`，表现时长 `0.5 s`。
- `男爵之力`没有 `visual_action`，使用逻辑 Buff 表现。
...
已核对蓝红SRU原表，当前Attack1/2与移动对应；仅采纳双方共同的零混合死亡入口，部署与0.5秒死亡时长保留。
...
`visual_active_buff_scene` 使用 `res://assets/effects/baron_minion/melee_minion.tscn`，改用小兵专用身体覆层、受击反馈与对应持续粒子，炮车兵另接原始强化炮弹出膛/飞行/命中。只消费权威或 Snapshot 的 Buff 状态，不改变模型缩放、权威半径、命中时序。
