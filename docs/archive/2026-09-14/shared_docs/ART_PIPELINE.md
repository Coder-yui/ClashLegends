...
候选竞技场保存在 `assets/arena/rift_arena/rift_arena.tscn`，目前运行时仍使用 `assets/arena/arena_rift_v4.png`。候选场景的规格和 Blender 编辑流程见该目录 README；它只在独立预览中与单位/建筑共用正交相机。场景中不添加碰撞或导航，权威地面仍由 ArenaRules 格子常量定义。
...
7. 在 [开发工作台](../../../DEVELOPMENT_WORKBENCH.md) 模型页查片段、实战页查时序，目视双方阵营、部署/待机/移动/攻击/死亡、脚底、遮挡和朝向。
8. 完整新卡继续核对卡面，并按 `AUDIO_INTEGRATION.md` 给当前使用的表现/命中事件接入声音；音频不挂在模型动画方法轨道中，不反向控制权威时序。

只有素材本身需要组合、网格过滤、骨骼挂点映射或双模型切换时才增加角色专属包装脚本，并把说明放在 `assets/units/<card_id>/README.md`。
