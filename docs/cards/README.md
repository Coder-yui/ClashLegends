...
主动技能的金币消耗按强度使用 `0`、`1`、`2` 三档；单个单位的技能次数使用 `1` 或 `2` 次。高爆发、强控制或复合强化通常为 `2` 费 / `1` 次；低强度或功能型技能通常为 `0`～`1` 费 / `2` 次。冰冻是法术卡特例：放在主动槽时直接启用强化冰冻，强化额外花费为 `0`，次数随该张法术卡本次施放计算。
...
- 每个单位/建筑主动技能配置 `0`～`2` 金币消耗、单个单位 `1`～`2` 次最大使用次数和冷却时间；主动槽按钮显示金币与次数，冷却中的按钮显示剩余冷却。冰冻的强化效果直接跟随卡牌施放，不生成单位主动按钮。
