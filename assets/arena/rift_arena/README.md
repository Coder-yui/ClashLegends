...
/Applications/Godot.app/Contents/MacOS/Godot --path . --script tools/capture/capture_rift_arena_v2.gd
```
...
```sh
/Applications/Godot.app/Contents/MacOS/Godot --path . --script tools/capture/capture_rift_arena_v2.gd -- --hold
```

`--hold` 保持手动推进后的比赛暂停，关闭窗口退出。交互式模型检查使用：

```sh
/Applications/Godot.app/Contents/MacOS/Godot --path . --script tools/capture/capture_rift_arena_v2.gd -- --inspect
```
