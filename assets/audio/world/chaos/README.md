...
- `Play_sfx_ENV_ChaosTurret_break03`：1 个导出变体。
- `Play_sfx_Env_Global_EoG_ChaosNexus_death_cast`：1 个导出变体。
- `Play_sfx_Env_map11_ChaosTurretMinionBasicAttack_cast`：3 个导出变体。
- `Play_sfx_Env_map11_ChaosTurretMinionBasicAttack_hit`：4 个导出变体。
- `Play_sfx_Env_map11_ChaosTurretMinionBasicAttack_missilelaunch`：4 个导出变体。
- `Play_sfx_Env_sruap_chaos_nexus_spawn`：1 个导出变体。
...
防御塔分阶段破损：damage:stage1 → Play_sfx_ENV_ChaosTurret_break01；damage:stage2 → break02；death 保留 break03。阈值与模型共用 PresentationConfig.structure_damage_stage，跨阶段只播放最终目标阶段，首次附着不补历史阶段。
