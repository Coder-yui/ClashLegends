...
| 形态/阵营 | 配置的事件与声音池 |
| --- | --- |
| 基础 | `deploy:voice`、`active_buff:end`、`active_buff:start`、`active_buff:sustain`、`attack_launch`、`attack_missile_cast`、`death`、`first_strike:cast`、`first_strike:hit_location`、`first_strike:missile_cast`、`first_strike:missile_launch`、`attack_swing`、`attack_hit`、`first_strike_hit` |

## 源文件与加工证据

- [event_manifest.json](event_manifest.json)：文件/变体、原始事件与加工来源。

部署语音使用用户指定的 `Play_vo_MissFortune_Move2DStandard` r1 短语音。
