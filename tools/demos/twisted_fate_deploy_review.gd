...
func _run() -> void:
	...
	main._audio_manager.cue_played.connect(func(id, cue, _pos):
		if id == "twisted_fate": print("[TF] ", main.mode, " t=", (Time.get_ticks_msec()-start)/1000.0, " ", cue))
	...
	if main.mode != "client":
		...
		main.play_card(0, "twisted_fate", Vector2(280, 840), {"immediate": true, "validate_position": false})
		main.play_card(1, "twisted_fate", Vector2(420, 820), {"immediate": true, "validate_position": false})
		if not network:
	...
	while elapsed < 5.0:
		...
		for unit in get_nodes_in_group("combatants"):
			if unit is Unit and unit.card_id == "twisted_fate" and not seen.has(unit.team):
				seen[unit.team] = true
				print("[TF appeared] ", main.mode, " team=", unit.team, " deploy=", unit._deploy_timer, " ready=", unit.is_deployed())
				if not network:
					await RenderingServer.frame_post_draw
