...
| [ashe_volley_collision_preview.gd](ashe_volley_collision_preview.gd) | 实际渲染非穿透 W：前排挡后排、侧箭继续。固定模拟，输出 /tmp/clash-ashe-volley-collision。 |
...
| [card_audio_batch_demo.gd](card_audio_batch_demo.gd) | 正式工作台出牌/技能事件的可复现录音；输出到 /tmp/clash-card-audio。 |
...
| [maintenance_preview.gd](maintenance_preview.gd) | 实际渲染 QA 工具，不是自动 mechanics 入口。截图输出 /tmp/clash-maintenance-render。 |
| [match_audio_review.gd](match_audio_review.gd) | match audio review |
| [missfortune_audio_demo.gd](missfortune_audio_demo.gd) | 录音输出到 /tmp/clash_missfortune_audio.wav，覆盖普攻、W 和死亡。 |
...
| [workbench_preview.gd](workbench_preview.gd) | 非 headless 运行；生成 /tmp/clash-workbench-*.png，并核对工作区暂停边界。 |
| [workbench_scenarios_preview.gd](workbench_scenarios_preview.gd) | 工作台场景与控件验收 |
| [xin_sweep_effect_review.gd](xin_sweep_effect_review.gd) | 实际工作台双阵营部署/主动横扫截图；输出 /tmp/clash-xin-sweep-review。 |
