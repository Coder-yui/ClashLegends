...
func run(harness: Object) -> void:
	...
	harness._expect(accepted and main._commands.card_commands.size() == 2 and main._elixir.elixir == 3, "0秒 A、0.25秒 B 重叠请求均接受扣费，等待生成不预留占地")
	...
	harness._expect(buildings.size() == 1 and buildings[0].global_position == pos and main._commands.card_commands.size() == 1, "0.5秒 A 原地生成，B 继续等待")
	...
	main._tick_pending_card_deployments(0.05)
	harness._expect(main._commands.card_commands.is_empty() and harness.get_nodes_in_group("combatants").filter(func(c): return c is Unit and c.is_building).size() == 2, "到期命令仅兑现一次")
	# 预部署阶段同样在真正生成时重查。
	main._commands.pre_deployments.append({"team": 0, "card_id": "apex_turret", "pos": pos, "time_left": 0.1})
	...
	harness._expect(waiting.is_empty() and main._commands.pre_deployments.size() == 1, "全场无合法落点时保留命令等待")
	blocker.free()
	main._tick_pending_card_pre_deployments(0.05)
	harness._expect(main._commands.pre_deployments.is_empty() and harness.get_nodes_in_group("combatants").filter(func(c): return c is Unit and c.is_building).size() == 1, "空间恢复后自动生成一次")
